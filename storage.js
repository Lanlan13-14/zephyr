...
function init({ hashPassword }) {
    ...
    addColumnIfMissing('connections', 'icon', "TEXT DEFAULT 'auto'");
    addColumnIfMissing('connections', 'iconSource', "TEXT DEFAULT 'auto'");
...
function updateConnectionRow(conn) {
    ...
    db.prepare(`UPDATE connections SET name=@name, host=@host, port=@port, protocol=@protocol, username=@username, password=@password, privateKey=@privateKey, remark=@remark, tags=@tags, connectionMode=@connectionMode, proxyId=@proxyId, jumpHostId=@jumpHostId, jumpHostIds=@jumpHostIds, sshKeyId=@sshKeyId, icon=@icon, iconSource=@iconSource, rdpSoundMode=@rdpSoundMode, rdpClipboard=@rdpClipboard, rdpMicrophone=@rdpMicrophone, rdpCamera=@rdpCamera, rdpStorage=@rdpStorage, rdpLocation=@rdpLocation, rdpResolution=@rdpResolution, rdpQuality=@rdpQuality, rdpFps=@rdpFps, rdpPipeline=@rdpPipeline, rdpTouchMode=@rdpTouchMode, rdpTouchSensitivity=@rdpTouchSensitivity, rdpDomain=@rdpDomain, ephemeral=@ephemeral, encoding=@encoding, visibility=@visibility, updatedAt=@updatedAt, revision=@revision, lastConnectedAt=@lastConnectedAt WHERE id=@id`).run(safe);
    return getConnectionById(conn.id);
}
