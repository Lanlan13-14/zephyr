...
function rowToConnection(row) {
    ...
    return { ...plain, port: Number(plain.port) || 22, tags: json(plain.tags, []), jumpHostIds: json(plain.jumpHostIds, plain.jumpHostId ? [plain.jumpHostId] : []), sshKeyId: plain.sshKeyId || '', lastConnectedAt: plain.lastConnectedAt || null, rdpSoundMode: plain.rdpSoundMode || 'local', rdpClipboard: plain.rdpClipboard === 0 ? false : true, rdpMicrophone: !!plain.rdpMicrophone, rdpCamera: !!plain.rdpCamera, rdpStorage: !!plain.rdpStorage, rdpLocation: !!plain.rdpLocation, rdpResolution: plain.rdpResolution || '1080p', rdpQuality: plain.rdpQuality || 'balanced', rdpFps: Number(plain.rdpFps) || 30, rdpPipeline: 'worker-gpu-v2', rdpTouchMode: plain.rdpTouchMode === 'relative' ? 'relative' : 'direct', rdpTouchSensitivity: Math.max(0.5, Math.min(3, Number(plain.rdpTouchSensitivity) || 1.5)), rdpDomain: plain.rdpDomain || '', ephemeral: !!plain.ephemeral };
}
