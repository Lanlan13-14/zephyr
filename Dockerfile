...
# ============================================================
# Stage 2: ffmpeg-builder — 编译已修复 CVE-2026-8461 的 FFmpeg
# ============================================================
...
RUN apk add --no-cache \
        build-base pkgconf curl xz \
        zlib-dev bzip2-dev xz-dev \
        x264-dev opus-dev libwebp-dev libxcb-dev pulseaudio-dev
...
RUN ./configure \
    ...
    && test "$(/opt/ffmpeg/bin/ffmpeg -hide_banner -version | awk 'NR==1{print $3}')" = "${FFMPEG_VERSION}"
...
RUN apk add --no-cache \
        ...
        x264-dev opus-dev libwebp-dev cairo-dev \
        zlib-dev
...
RUN apk add --no-cache \
        ...
        openssl-dev opus-dev x264-dev cjson-dev \
        krb5-dev pcsc-lite-dev fuse3-dev libwebp-dev cairo-dev

COPY --from=ffmpeg-builder /opt/ffmpeg /opt/ffmpeg
COPY --from=freerdp3-builder /opt/freerdp3 /opt/freerdp3
ENV PKG_CONFIG_PATH=/opt/ffmpeg/lib/pkgconfig:/opt/freerdp3/lib/pkgconfig
ENV LD_LIBRARY_PATH=/opt/ffmpeg/lib:/opt/freerdp3/lib
...
RUN echo "=== runtime diagnostics ===" && \
    ...
    test "$(ffmpeg -hide_banner -version | awk 'NR==1{print $3}')" = "8.1.2" && \
    for lib in /opt/ffmpeg/lib/libavcodec.so /opt/ffmpeg/lib/libavformat.so /usr/local/lib/librdp_bridge.so /opt/freerdp3/lib/libfreerdp3.so.3 /opt/freerdp3/lib/libwinpr3.so.3 /opt/freerdp3/lib/freerdp3/librdpsnd-client-bridge.so; do ldd "$lib"; done && \
    python -c "import ctypes; ctypes.CDLL('/opt/ffmpeg/lib/libavcodec.so'); ctypes.CDLL('/usr/local/lib/librdp_bridge.so'); print('ffmpeg and librdp_bridge loaded')" && \
