...
public final class TerminalRow {
    ...
    public char charAt(int index) {
        return (index >= 0 && index < mSpaceUsed) ? mText[index] : ' ';
    }

    public int getCodePoint(int column) {
        int charIndex = findStartOfColumn(column);
        return (charIndex >= 0 && charIndex < mSpaceUsed) ? Character.codePointAt(mText, charIndex) : ' ';
    }
