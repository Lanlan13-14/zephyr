 ...
 * copied into the staged core by scripts/stage-native-rdp.mjs.
 *
 * The markup (#rdpStorage, #rdpStorageFolder, #rdpStorageDeviceName,
 * #rdpStorageFolderPickBtn) already exists in the shared app.html but was purely
 * presentational: the pick button had no handler and neither field was ever read
 * or written. This file connects all three ends.
 *
 * Why it lives outside app.js:
 *   A mapped folder is an absolute path on *this* machine, handed to FreeRDP's
 *   RDPDR drive redirection. The browser product has no such concept — it cannot
 *   even learn a real path, since showDirectoryPicker() yields an opaque handle.
 *   Putting the fields in the shared connection record would therefore add a
 *   field the browser can never populate, so the mapping is stored One-side
 *   under /api/one/rdp/storage-mapping/:connectionId and the shared schema is
 *   left untouched.
 *
 ...
 *   as far as Tauri is concerned and cannot invoke commands. Granting the
 *   loopback origin IPC access would hand it to any process that can bind a
 *   local port. Instead the page files a request, the Rust shell polls for it,
 *   opens the OS dialog, and posts the chosen path back. Same pattern the shell
 *   already uses to follow the theme.
 *
 * Reading #connectionId rather than app.js's `editingId`:
 *   app.js keeps `editingId` in module scope, but mirrors it into the
 *   #connectionId input on every modal open (app.js:2961). The input is a public
 *   contract; the variable is not.
 */
'use strict';
...
(function () {
        ...
        return new Promise(function (resolve, reject) {
            function tick() {
                ...
                fetch('/api/one/rdp/pick-folder/' + encodeURIComponent(id), {
                    ...
                    if (data.status === 'done') { resolve(data.path || ''); return; }
                    reject(new Error(data.error || '未能获取所选文件夹'));
                }).catch(function () {
    ...
    function pickFolder() {
        ...
        fetch('/api/one/rdp/pick-folder', {
             ...
             * user just chose, and an empty device name is the most common way
             * to end up with a mapping FreeRDP refuses. Never overwrite a name
             * the user typed. */
     ...
     * store. Wrapping fetch means the mapping is attached using the *public*
     * HTTP contract (`POST /api/connections` → `{ id }`), so nothing here breaks
     * when app.js's internals change.
     *
    ...
    function init() {
        ...
        /* Clearing the folder disables the mapping outright, so keep the
         * device-name field honest about it. */
