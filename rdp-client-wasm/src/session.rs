...
use crate::audio::ZephyrRdpsndBackend;
use crate::canvas::Canvas;
use crate::drive::ZephyrDriveBackend;
use crate::clipboard;
...
impl iron_remote_desktop::SessionBuilder for SessionBuilder {
    ...
    fn render_canvas(&self, canvas: HtmlCanvasElement) -> Self {
        self.0.borrow_mut().render_canvas = Some(canvas.into());
        self.clone()
    }
    ...
    async fn connect(&self) -> Result<Self::Session, Self::Error> {
...

        {
            ...
            render_canvas = inner.render_canvas.clone().unwrap_or(JsValue::NULL);
...
async fn connect(
    ...
    // Always attach RDPSND with ZephyrRdpsndBackend so the server sends audio.
    // Windows RDPDR also requires RDPSND to be advertised (MS-RDPEFS Appendix A<1>).
    connector.attach_static_channel(Rdpsnd::new(Box::new(ZephyrRdpsndBackend::new())));

    if let Some(printer_backend) = printer_backend {
        connector.attach_static_channel(
            Rdpdr::new(Box::new(printer_backend), computer_name.clone()).with_printer_driver(
                printer_device_id,
                printer_name,
                printer_driver_name,
            ),
    ...
    // Attach drive redirection if a drive backend was registered.
    if let Some(drive_backend) = crate::drive::take_pending_drive_backend() {
        tracing::info!("Attaching RDPEFS drive redirection backend");
        connector.attach_static_channel(
            Rdpdr::new(Box::new(drive_backend), computer_name.clone()),
        );
    }
    ...
    // Zephyr's /rdp-proxy is a raw WebSocket -> TCP bridge, not an RDCleanPath gateway.
    // Drive the normal X.224/NLA connector directly over the WebSocket stream and mark
    // the browser transport as already upgraded. TLS/certificate probing is handled by
    // Zephyr's Node side before the browser session starts.
    let mut begin_buf = WriteBuf::new();
    while !connector.should_perform_security_upgrade() {
        ironrdp_futures::single_sequence_step(&mut framed, &mut connector, &mut begin_buf).await?;
    }
