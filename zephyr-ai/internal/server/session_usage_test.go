...
func TestSessionUsageEndpointIsUserScoped(t *testing.T) {
	...
	defer store.Close()
	sess, _ := store.CreateSession("u1", "usage", nil, "generation-1")
	...
	defer srv.Close()
	req := httptest.NewRequest(http.MethodGet, "/admin/sessions/"+sess.ID+"/usage?userId=u1&databaseGeneration=generation-1", nil)
