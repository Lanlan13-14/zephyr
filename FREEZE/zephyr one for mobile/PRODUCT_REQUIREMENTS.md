...
- [KNOWN] 绑定 Zephyr 主端和文件同步均为可选能力，不是启动 App、进入首页或使用设备本地连接库的前置条件。
- [KNOWN] 首次启动和未绑定状态必须直接进入可用的本地工作区；用户可创建、编辑、复制、删除和使用设备本地数据，并可使用不依赖主端的本地设置。
- [KNOWN] 未绑定、离线、Token 缺失、登录失败、TOTP 失败、同步冲突或同步服务不可用时，只能影响主端绑定/同步及明确依赖主端的能力，不得把整个 App 变成阻断页或不可操作空壳。
- [KNOWN] 用户主动开启文件同步时，才执行以下账号、TOTP 和 Client Token 前置检查；关闭同步后本地工作区仍可继续使用。
