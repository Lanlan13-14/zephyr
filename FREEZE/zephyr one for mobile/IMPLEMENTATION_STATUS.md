...
| F-027 | iOS security-scoped bridge | `missing` | `partial` | [KNOWN] security-scoped provider 与书签授权链已实现: `ZephyrCore/SecurityScopedFileProvider` / `PosixSecurityScopedFileSystem` / `SecurityScopedGrants` / `VirtualPath`, 57 个 XCTest + 19 静态 gate (`ios-security-scoped-provider.test.mjs`), 48 mutations / 0 survivors; 路径监禁采用 realpath 规范化后重查包含关系，拒绝路径中间的软链逃逸; 仍缺文档选择器接线 (`onPickDriveDirectory`) 与真机验证; 本机无法编译 Swift |
