...
| machine contracts | `specified` | [KNOWN] 20 实体／66 错误码／15 份 codegen 产物；`zephyr_one/mobile/tests` 183 项全过，`check:drift` 无漂移 |
| Android Kotlin/Compose | `partial` | [KNOWN] `zephyr_one/mobile/android` 21 模块、295 个 `.kt`，150 个 `@Composable`、7/23 屏幕；`ZephyrOneRoot` / `AccountContainer` / `ZephyrApplication` / `SecretStore.SecretScope` 四处未声明引用已补齐，并由 `mobile/tests/kotlin-symbols.test.mjs` 看守；手写 wire DTO 与冻结 OpenAPI 的 10 处不一致已修正（`ownerLabel`→`ownerDisplayName`、`{ok,resources}`→`{items,nextPageToken}`、`purpose/requestDirect`→`mode/clientSessionNonce/requestedChannels/deviceKeyVersion`、`relayUrl`→`relay{websocketUrl,protocol,credential}`、file-bridge 请求体→`shareProfileIds`），现由 `mobile/tests/kotlin-dto-contract.test.mjs` 逐字段钉住 schema；仍无 Gradle wrapper；CI `zephyr-one-mobile.yml` 的 `android` job 已在 temurin 17 + Android SDK 35 上成功编译 `assembleDebug` 并通过 `testDebugUnitTest`（2026-08-13）；未绑定服务器时以设备本地 workspace 直接打开，同步可选、不阻塞使用（`AppContainer.ensureLocalWorkspace` + `AccountContainer.localMode`） |
| iOS Swift/SwiftUI | `partial` | [KNOWN] `zephyr_one/mobile/ios` 现有 `Package.swift`（ZephyrContracts / ZephyrCore / ZephyrCoreTests 三个 target，无第三方依赖）；`MobileAad.swift` 与 `Zft2Codec.swift` / `Zft2Meta.swift` 已按 Kotlin 同源移植，XCTest 套件直读 `contracts/generated/aad-vectors.json`（17 例）与 `zft2-frames.json`（5 帧 + 5 拒绝）；算法已用同构实现逐字节比对全部 fixture 通过，静态关口为 `mobile/tests/swift-symbols.test.mjs`；**仍未用 swiftc 编译**（新增 `zephyr-one-mobile.yml` 的 macOS `ios` job 负责，本地 Windows 无 Swift 工具链）；无 Xcode 工程、无 SwiftUI App 代码、无屏幕 |
| mobile v1 server API | `partial` | [KNOWN] `mobile-v1-routes.js` / `mobile-v1-store.js` / `mobile-v1-entities.js` / `mobile-v1-shared.js` / `mobile-v1-crypto.js`?? `server.js` ???20 ?????? 19 ?????? e2e ???capabilities / bind / refresh / devices CRUD / bootstrap / changes / push / ack / now / status / sensitive.verify / shared ????? / invoke / session open+refresh+close?`relay-strict` ? SSH ???????? `/api/mobile/v1/shared/relay`?device ?????????????????????????RDP/VNC relay ??????? `shared_relay_unavailable`????????? `POST /file-bridge/lease`???? SAF/security-scoped provider ? ZFT2 lease ??????????? 501 `unsupported_scope` ?????????? success ???`capabilities.features.fileBridge=false`?`mobile/tests` 165 ????22 ? sync ?? + 30 ? shared residency? |
...
| F-026 | Android SAF foreground bridge | `missing` | `partial` | [KNOWN] SAF provider 与授权链已实现: `feature-file-sync` / `SafZft2FileProvider` / `ContentResolverDocumentTree` / `SafShareGrants` / `PersistentShareStore` / `FileSyncShareCoordinator`, 62 个 JVM 单测 + 25 静态 gate (`android-saf-provider.test.mjs`), 54 mutations / 0 survivors; 路径监禁采用子节点递归解析，不做 document id 拼接; 目录选择器已接线（SAF picker -> 持久化授权 -> RDP drive profile），恢复前台时重新校验授权; 仍缺真机验证; 本机无法编译 Kotlin |
| F-027 | iOS security-scoped bridge | `missing` | `partial` | [KNOWN] security-scoped provider 与书签授权链已实现: `ZephyrCore/SecurityScopedFileProvider` / `PosixSecurityScopedFileSystem` / `SecurityScopedGrants` / `VirtualPath` / `PersistentBookmarkStore` / `PersistentShareRowStore`, 72 个 XCTest + 27 静态 gate (`ios-security-scoped-provider.test.mjs`), 72 mutations / 0 survivors; 路径监禁采用 realpath 规范化后重查包含关系，拒绝路径中间的软链逃逸; 授权已持久化 (`PersistentBookmarkStore` + `PersistentShareRowStore`，重启后保留目录); 仍缺文档选择器接线 (`onPickDriveDirectory`) 与真机验证; 本机无法编译 Swift |
...
| F-038 | Shared strict relay | `implemented-zephyr` | `missing` | [KNOWN] SSH relay ????`/api/mobile/v1/shared/relay` ??????????????? `resolveForConnect` ?? upstream?????? PTY ??? `accepted`/`ready`/`error`/`close`/`revoked` ?????????????input ?? ACL ?? terminal ?????? 5s ???????????????????????????????????? sync bearer???????? host ???RDP/VNC ???????????? `shared_relay_unavailable`??? One relay ????? |
