...
} from './wterm-xterm-fit.js?v=20260728-ai-handle-only-drag1';
...
function styleTerminalToastNode(el) {
    ...
    // absolute + Motion y，避免 flex 流双倍间距
...
function sendMobileStableImeText(text = '', source = 'mobile-ime', { paste = false, forceImmediate = false } = {}) {
    ...
    // A repeated printable character is valid input (the second "t" in
    // "nexttrace" must not be treated as a duplicate "t"). Only an equal
    // payload crossing event paths — the beforeinput timeout fallback being
    // re-delivered by the authoritative input event — is suppressed.
    if (mobileImeLastSent.text
        && imeTextEqual(mobileImeLastSent.text, payload)
...
function setupPanelLayoutMenu() {
    ...
    document.querySelectorAll('[data-layout-panel]').forEach((button) => {
        ...
        button.addEventListener('click', (e) => {
            ...
            e.stopPropagation();
            if (consumeLayoutClickSuppression()) return;
            // Local flag still used by editor-only paths that haven't migrated yet.
