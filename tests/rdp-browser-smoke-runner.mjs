...
const chrome=spawn(chromium,['--headless','--no-sandbox','--disable-dev-shm-usage','--use-gl=angle','--use-angle=swiftshader','--enable-unsafe-swiftshader',`--remote-debugging-port=${debugPort}`,'about:blank'],{stdio:'ignore'});
...
async function cdp(){for(let i=0;i<50;i++){try{const p=(await json(`http://127.0.0.1:${debugPort}/json`))[0];if(p)return new WebSocket(p.webSocketDebuggerUrl)}catch{}await wait(100)}throw new Error('CDP unavailable')}
