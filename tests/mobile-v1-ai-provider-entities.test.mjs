...
test('second-round Android provider patches accept nested config paths and flattened context windows', () => {
  const context = fresh();
  try {
    const alice = { userId: 'alice' };
    context.adapter.create(alice, 'provider-second-round', {
      name: 'Round one',
      type: 'openai-compatible',
      baseUrl: 'https://api.example.invalid/v1',
      defaultModel: 'gpt-4o',
      models: [{ id: 'gpt-4o', label: 'GPT-4o' }],
      config: { apiMode: 'chat', options: { vision: true, max_tokens: 2048 } },
    });

    const { assertMaskAllowed } = require(path.join(root, 'mobile-v1-entities.js'));
    const spec = enabledRegistry.entities.find((entity) => entity.type === 'aiProvider');
    ...
    assert.throws(
      () => assertMaskAllowed(spec, ['apiKey']),
      (error) => error.code === 'invalid_request',
    );
    assert.throws(
      () => assertMaskAllowed(spec, ['ownerUserId']),
      (error) => error.code === 'invalid_request',
    );
    assert.throws(
      () => assertMaskAllowed(spec, ['config.__proto__.polluted']),
      (error) => error.code === 'invalid_request',
    );
    assert.throws(
      () => assertMaskAllowed(spec, ['unknownLeaf']),
      (error) => error.code === 'invalid_request',
    );

    const numericContext = context.adapter.update(alice, 'provider-second-round', {
      config: { options: { context: 128000, vision: false } },
    });
    assert.equal(numericContext.config.options.context.windowTokens, 128000);
    assert.equal(numericContext.config.options.vision, false);

    const flattenedTokens = context.adapter.update(alice, 'provider-second-round', {
      config: { options: { windowTokens: 64000 } },
    });
    assert.equal(flattenedTokens.config.options.context.windowTokens, 64000);

    const nestedObject = context.adapter.update(alice, 'provider-second-round', {
      config: { options: { context: { windowTokens: 32000 } } },
    });
    assert.equal(nestedObject.config.options.context.windowTokens, 32000);
