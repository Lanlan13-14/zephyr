...
test('app HTML ships the current i18n cache-busting revision', async () => {
    ...
    assert.match(html, /app\.js\?v=20260727-ai-confirm-fix1/);
    assert.match(html, /i18n\/runtime\.js\?v=20260727-ai-confirm-fix1/);
});

test('locale switch hook repaints state-derived security fragments', async () => {
    const source = await (await fetch(server.url('/app.js?v=20260727-ai-confirm-fix1'))).text();
