...
**项目仍处于随缘开发阶段，请谨慎用于生产环境；生产部署务必持久化 `/app/data` 并自行做好访问控制、备份与回滚。RDP 已切换到 FreeRDP3 RDPEGFX native bridge 路线，但不同 Windows 版本、浏览器 WebCodecs 能力、网络质量和显卡/CPU 条件会影响体验，建议先在测试环境验证。**
---
